...
from .project_manager import PROJECT_FILE_NAME, SCHEMA_VERSION
...
# ── Dataset metadata helpers (data.yaml / classes.txt / project.json) ──

def _scan_yolo_class_ids(labels_dir: Path) -> set:
    ids = set()
...
def _load_class_names(ds: Path) -> list:
    """Best-effort parse of class names from data.yaml or classes.txt."""
    yml = ds / "data.yaml"
    if yml.exists():
        try:
            import yaml
            ...
            if isinstance(names, dict):
                try:
                    return [
                        str(names[k]) for k in sorted(names, key=lambda x: int(x))
                    ]
...
def _write_xanylabeling_project(out_dir: Path, names: list) -> None:
    """Write a project config so the folder can be opened as a project."""
    ...
    project = {
        ...
        "name": out_dir.name,
        "description": "Generated by data augmentation",
        "created": now,
        "modified": now,
        "version": "1.0",
        "paths": {
            "images_dir": "images",
            # YOLO labels live in labels/; the project can still be
            # opened, and the existing Import Dataset flow will pick up
            # data.yaml from the same folder when needed.
    ...
    (out_dir / PROJECT_FILE_NAME).write_text(
        json.dumps(project, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )


def ensure_dataset_files(src_dir: Path, out_dir: Path) -> list:
    """Ensure ``out_dir`` is a self-contained YOLO dataset + X-AnyLabeling project.

    Copies/synthesises ``data.yaml`` and ``classes.txt`` and writes the
    project config so the folder is importable as a dataset *and* openable
    as a project. Returns the resolved class names.
    """
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    # 1. Resolve class names: prefer source files, then scan labels, else empty.
    names = _load_class_names(Path(src_dir))
    if not names:
        ids = _scan_yolo_class_ids(out_dir / "labels")
        if ids:
            names = [f"class_{i}" for i in range(max(ids) + 1)]

    # 2. Copy existing data.yaml / classes.txt if present;
    #    otherwise synthesise minimal versions.
    src = Path(src_dir)
    yml_src = src / "data.yaml"
    if yml_src.exists():
        shutil.copy2(yml_src, out_dir / "data.yaml")
    else:
        yaml_lines = [
    ...
    # 3. Always write an X-AnyLabeling project file so the folder
    #    can be opened directly as a project.
    try:
        _write_xanylabeling_project(out_dir, names)
    except OSError:
        pass
